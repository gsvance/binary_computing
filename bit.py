...
class Bit:
	...
	def __init__(self, state=None):
		"Create a new bit, optionally initializing its value with a boolean."
	...
	def __nonzero__(self):
		"Converts a bit to a bool value and makes Bit(Bit()) work properly."
...
if __name__ == "__main__":
	...
	samples = 10**6
	bits = [Bit() for i in xrange(samples)]
	mean = sum([(1 if b == One else 0) for b in bits]) / float(samples)
